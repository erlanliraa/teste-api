...
import { toolsRoadMap, toolsHost, toolsBudget, toolsTips, promptTemplate, toolsRestaurants, toolsSymbol} from'../templates/prompt-template';
...
async function generateRoteiro(prompt: string, apiKey: string): Promise<string> {
    ...
    try {
      const response = await openai.chat.completions.create({
        ...
        tools: [toolsRoadMap, toolsHost, toolsBudget, toolsTips, toolsRestaurants, toolsSymbol]
      });
  
      const json_output = formatTravelPlan(response.choices[0]?.message.tool_calls || [])
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  console.log("TROUXE: ", travel_period,
    budget,
    destination,
    preferred_travel_styles)
